...
async function cmdFlow(i, categories, numberOfCategories) {
  ...
  const r3 = await showTicketCategorySelector(
    r2,
    'Select a category to create a ticket in.',
  );
  const r4 = await showTicketCategorySelector(
    r3,
    'Select a category for closed tickets to go.',
  );
  ...
  const obj = {
    category,
    categoryClose,
    ticketResponseId: r4.ticketResponseId,
    iNew: r4.iNew,
  };
